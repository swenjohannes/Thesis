...
def discrete_grid_no_income(Re, Rf, Z, gamma = 5, G = 50, basis_order = [2, 0, True, False, False], method = 'ols',
                            transform = False, use_test_port = False, test_ratio = 0.2):
    ...
    #gamma = 15; G = 100; basis_order = None; method = 'ols'; transform = True; use_test_port = True; test_ratio = 0.1
    K, T, M = Re.shape
    ...
    for t in reversed(range(1, T)):
        ...
        if use_test_port:
            x_max = np.zeros(M)
            for m in range(M):
                psi_est = fast_ols(psi_g[:, m], x_test_basis, x_new = x_full_basis)
                x_max[m] = np.argmax(psi_est)
            x_opt = x_full[x_max.astype(int)].T
        else:
